...
def clustering(dataframes_input, columns_to_cluster, weights, k):
    for quarter in dataframes_input:
        ...
        for i in range(1, len(dataframes_input[quarter])): # i for Industry Based Cluster
            ...
            while (any(dataframes_input[quarter][i].iloc[:, -1].value_counts() > 10)):
...
# INPUT: Company GV_Keyas well as grouped_dfs from previous method (Dictionary with quarters and clusters)
...
def get_multiples(company, quarter, peer_groups_every_quarter,raw_data,multiple_type):
    ...
    for comp in peer_group:
        multiple = raw_data[quarter][raw_data[quarter]["GV_Key"] == comp][multiple_type].values[0]
...
#get peer group companies' mean multiples as a separate dataframe to then be joined with valuation df
    ...
    for quarter in raw_data:
        peers = {"P/EBIT AVG": [],"P/EBITDA AVG":[],"P/Sales AVG":[],"P/E AVG":[], "Index": []}
        
        print(quarter)
        for i in raw_data[quarter].index:
            for m in ["P/EBIT","P/EBITDA","P/Sales","P/E"]:

                peers[m + " AVG"].append(get_multiples(raw_data[quarter].loc[i, "GV_Key"], raw_data[quarter].loc[i,"Quarter"],peer_groups_every_quarter,raw_data,m))
...
def old_trading_strategy (valuation_df):
    ...
    for quarter in valuation_df:
        ...
        companies = valuation_df[quarter]["GV_Key"].unique()
        data_companies = {}
        returns = []
        ...
        for company in companies:
            ...
            data_one_company = valuation_df[valuation_df['GV_Key'] == company].copy()
...
def trading_strategy (valuation_df):
    ...
    for company in companies:
        ...
        print('Trading strategy for company', company, status)
            ...
            #print(np.where(status == round(status, 0)*100 % 2 == 0))
        data_one_company = data[data['GV_Key'] == company].copy()
...
columns_to_cluster = ['Assets', 'HQ', 'Beta_2Y', 'Revenue', 'Div_Yield','Rev_1y_Growth', 'Industry Group',
                      'EBITDA_Margin','Profit_Margin', 'Leverage', 'Debt/Assets',
                      'Cash/Assets', 'ROE', 'P/B', 'P/Sales', 'P/EBITDA', 'P/EBIT', 'P/E']

comb = [1/(len(columns_to_cluster))] * len(columns_to_cluster)
combinations = [comb]


#only first 100 companies for testing purposes
raw_data_100 = input(
    r"C:\Users\danie\OneDrive - Wirtschaftsuniversität Wien - IT-SERVICES\Dokumente\WU\Bachelorarbeit\02 Code\03 Input Data\clustering_data_scaled.csv")

#perform initial clustering
primary_industry_clusters = first_clustering(raw_data_100)

### testing without optimization
primary_industries_clustered = clustering(primary_industry_clusters, columns_to_cluster, comb, k = 2)
...
valuation_df = optimization(raw_data_100, columns_to_cluster, combinations)
